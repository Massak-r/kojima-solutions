...
export function QuickCaptureFab({ projectHint }: QuickCaptureFabProps) {
  ...
  return (
    <>
      {/* FAB — stacked above QuickActionFAB. Vertical offsets match the FAB
        *  stack rules: 14h primary at bottom-24 (mobile) / bottom-8 (desktop),
        *  capture sits at primary-top + 12px gap. Stays clear of the mobile
        *  BottomNav (h-16). */}
      <button
        onClick={() => setOpen(true)}
        className={cn(
          "fixed right-4 z-40 bottom-44 md:bottom-28 md:right-6 no-print",
