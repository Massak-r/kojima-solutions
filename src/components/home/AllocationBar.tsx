  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <p className="text-xs font-mono text-primary font-semibold">{TOTAL} {t("crédits", "credits")}</p>
      </div>
                ...
                >
                  <div className="bg-background p-2.5 rounded-lg shadow-xl border border-border/60 text-center backdrop-blur-xl">
                    ...
                    <p className="text-[10px] font-mono text-primary font-semibold mt-0.5">
                      {seg.units} {t("crédit" + (seg.units > 1 ? "s" : ""), "credit" + (seg.units > 1 ? "s" : ""))}
                    </p>
                  </div>
                </motion.div>
              )}
      ...
      <div className="flex flex-wrap gap-x-3 sm:gap-x-5 gap-y-1.5 justify-center mb-4">
        {SEGMENTS.map((seg, i) => (
          ...
          >
            ...
            <span className="text-muted-foreground/50 font-mono">({seg.units}h)</span>
          </button>
        ))}
      </div>
      ...
      <div className="border-t border-border/30 pt-4">
        <button
          ...
          className="flex items-center gap-2 text-xs font-display font-semibold text-primary hover:text-primary/80 transition-colors w-full"
        >
          <span>{t("Exemple de projet par étape", "Project example by phase")}</span>
          <ChevronDown className={cn("w-3.5 h-3.5 transition-transform duration-300", showExample && "rotate-180")} />
          <span className="ml-auto font-mono text-xs text-primary/60">{PROJECT_TOTAL}h</span>
        </button>
            ...
            >
              <div className="pt-3 space-y-3">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="ml-auto text-xs font-mono text-muted-foreground/60">{phaseTotal}h</span>
                      </div>
                      <div className="ml-5 pl-3 border-l-2 border-border/20 space-y-1">
                        {phase.tasks.map((task, ti) => (
                          <div key={ti} className="flex items-center justify-between text-[11px] font-body text-muted-foreground">
                            ...
                            <span className="font-mono text-foreground/60 font-medium">{task.hours}h</span>
                          </div>
                        ))}
                      </div>
                    </motion.div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border/30 text-sm font-body">
                  ...
                  <span className="font-mono font-bold text-primary">{PROJECT_TOTAL}h = {(PROJECT_TOTAL * 100).toLocaleString("fr-CH")} CHF</span>
                </div>
              </div>
            </motion.div>
          )}
