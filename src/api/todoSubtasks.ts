...
/** List all subtasks, optionally filtered by parentId */
export function listSubtasks(parentId?: string) {
  const params = parentId
    ? `?source=admin&parent_id=${parentId}`
    : `?source=admin`;
...
export function createSubtask(data: {
  ...
  return apiFetch<SubtaskItem>('todo_subtasks.php', {
    method: 'POST',
    body: JSON.stringify({ ...data, source: 'admin' }),
  });
}

export function updateSubtask(
  id: string,
  data: Partial<Pick<SubtaskItem, 'completed' | 'text' | 'dueDate' | 'order' | 'description' | 'smartSpecific' | 'smartMeasurable' | 'smartAchievable' | 'smartRelevant' | 'priority' | 'status' | 'flaggedToday'>>,
) {
